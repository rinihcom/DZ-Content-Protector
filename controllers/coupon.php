...
class div_billing_coupon{
	...
	public function __construct() {
		...
		$this->title = 'Coupon ';
		$this->titles = 'Coupons ';
		$this->table = $wpdb->prefix.'div_billing_coupon';
	}
		
	public function manage(){
		// Edit screen
		$messages = array();
		$wp_list_table = new div_billing_coupon_lists();
	...
	public function edit(){
		$data = $this->wpdb->get_row( "SELECT * FROM ".$this->table." WHERE coupon_id='".$_GET['id']."'" );
		include_once( div_billing_pluginpath() . '/views/coupon_form.php' );
	}
	...
	private function validate(){
		$this->messages = array();
		if(empty($_POST['name'])){
			$this->messages[] = 'Please fill coupon code';
		}
...
$div_billing_coupon = new div_billing_coupon;
...
class div_billing_coupon_lists extends WP_List_Table {
	/**
	...
	*/
	 ...
	 */
	protected $user_id;
	protected $div_table = "div_billing_coupon";
	...
	public function get_sortable_columns() {
		$sortable_columns = array(
            'coupon_code'     => array('coupon_code',true),     //true means its already sorted
	...
	function column_coupon_code($item){
        ...
        //Build row actions
        $actions = array(
            'Edit'      => sprintf('<a href="?page=%s&action=%s&id=%s">Edit</a>',$_REQUEST['page'],'edit',$item['coupon_id']),
            'Delete Data'      => sprintf('<a href="?page=%s&action=%s&id=%s">Delete data</a>',$_REQUEST['page'],'delete',$item['coupon_id'])
        );
        
        //Return the title contents
        return sprintf('%1$s <span style="color:silver">(id:%2$s)</span>%3$s',
            /*$1%s*/ $item['coupon_code'],
            /*$2%s*/ $item['coupon_id'],
